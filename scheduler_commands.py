...
class SchedulerCommands:
    ...
    def handle_schedule(self, args: List[str]) -> None:
        """Handle schedule command to configure scheduling algorithm"""
        if len(args) < 2:
            raise ValueError("schedule: missing algorithm\nUsage: schedule <rr|priority> [time_quantum]")

        algorithm_str = args[1].lower()
        ...
        if algorithm_str == "rr" or algorithm_str == "round_robin":
            ...
            if len(args) > 2:
                try:
                    time_quantum = float(args[2])
                    if time_quantum <= 0:
                        raise ValueError("schedule: time quantum must be positive")
                except ValueError as e:
                    if "time quantum must be positive" in str(e):
                        raise e
                    raise ValueError(f"schedule: invalid time quantum '{args[2]}': must be a number")
            ...
            # Warn if there are existing processes that might conflict
            ...
            raise ValueError(f"schedule: unknown algorithm '{algorithm_str}'\nSupported algorithms: rr (round_robin), priority")
